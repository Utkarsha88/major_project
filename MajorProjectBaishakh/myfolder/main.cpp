...
static float safeSensorValue(float value)
{
    ...
    int B)
{
  if (WiFi.status() == WL_CONNECTED)
  {
    ...
    float safeTemp = safeSensorValue(temperature);
    float safeHumidity = safeSensorValue(humidity);
    float safePressure = safeSensorValue(pressure);
    float safeGas = safeSensorValue(gas);
    float safeDifference = safeSensorValue(gasDifference);
    float safeVocPercent = safeSensorValue(vocPercent);
    ...
    json += "\"temperature\":" + formatFloat(safeTemp, 2) + ",";
    json += "\"humidity\":" + formatFloat(safeHumidity, 2) + ",";
    json += "\"pressure\":" + formatFloat(safePressure, 2) + ",";

    json += "\"gasResistance\":" + formatFloat(safeGas, 2) + ",";

    json += "\"difference\":" + formatFloat(safeDifference, 2) + ",";

    json += "\"vocPercent\":" + formatFloat(safeVocPercent, 2);
